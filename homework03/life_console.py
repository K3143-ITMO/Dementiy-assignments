...
class Console(UI):
    """
    GameOfLife's subclass implementing console GUI
    """
    ...
    def draw_borders(self, screen) -> None:
        """ Отобразить рамку. """
        screen.border()  # in-built

    def draw_grid(self, screen) -> None:
        """ Отобразить состояние клеток. """
        for i in range(1, self.rows - 1):  # count for borders
            for j in range(1, self.cols - 1):  # count for borders
    ...
    def run(self) -> None:
        ...
        term.keypad(True)  # enable escape seqs
        curses.noecho()  # do not echo the input chars
        term.nodelay(True)  # do not require newlines
